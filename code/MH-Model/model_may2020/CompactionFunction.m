...
%         ell = 200/(omega*917*334000); % lengthscale
    ...
    case 'arthern'
        ...
        f = 1-(550/rhoi); % transition porosity
        Ec = 60e3; % activation energy
        Eg = 42.4e3; % activation energy
        T = 273.15+(Q0/h0)*Theta; % temperature in kelvin
        Tav = T; % for now, later needs to be calc'd in main
        k0 = 0.07; % arthern2010 constant
        k1 = 0.03; % arthern2010 constant
        b = (rhoi./rhow).*Accumulation.*ell; %revisit, not sure the units are right
        c0 = (k0.*(b.*g)).*exp(-Ec./(R*T)+Eg./(R*Tav));
        c1 = (k1.*(b.*g)).*exp(-Ec./(R*T)+Eg./(R*Tav));
        C = c0.*(phi>=f)+c1.*(phi<f);
        Shear = C.*(phi./(1-phi)); % dw/dz
end
end
